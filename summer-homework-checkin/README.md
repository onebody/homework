...
💡 **最新版本 v1.3** — Webhook 消息推送、钉钉双向机器人、全链路北京时间治理、站点动态配置（标题/标语），详见[版本记录](#八版本记录)。

📚 **完整文档**：见 [`docs/`](docs/README.md) —— [架构设计](docs/ARCHITECTURE.md) · [功能模块](docs/FEATURES.md) · [API 接口](docs/API.md) · [部署运维](docs/DEPLOYMENT.md) · [用户手册](docs/USER_GUIDE.md) · [变更记录](docs/CHANGELOG.md) · [项目总结](docs/PROJECT_SUMMARY.md)。
...
| 消息推送（v1.3） | 打卡提交/通过/拒绝实时推送钉钉/企微群机器人；钉钉群内 @机器人可查统计/待审核/直接审核（加签验签） |
| 站点动态配置（v1.3） | 学生端页面标题与登录页欢迎标语后台可配，置空恢复默认，刷新即生效 |
| 时间准确性（v1.3） | 全链路统一北京时间（容器 TZ + 应用层 now_local + 历史数据迁移平移） |
...
### v1.3（2026-07）
- **新增**：Webhook 消息推送模块（钉钉/企微群机器人，后台可配开关/事件/限频，异步不阻塞主流程）
- **新增**：钉钉双向机器人 —— 群内 @机器人查统计/待审核/查询学生/直接审核（HMAC 加签验签）
- **新增**：站点动态配置 —— 学生端标题与登录页欢迎标语后台可配（「⚙️ 系统设置」），置空恢复默认
- **改进**：全链路时区治理 —— 存储统一 naive 北京时间，容器 `TZ=Asia/Shanghai`，历史数据迁移整体 +8h 并重算打卡归属日期
- **新增**：项目总结文档 [docs/PROJECT_SUMMARY.md](docs/PROJECT_SUMMARY.md)

### v1.2 / v1.2.1（2026-07）
- **安全**：安全审计修复、全库敏感信息清理、`.gitignore` 门禁体系、双环境增量部署脚本

### v1.1（2026-07）
- **新增**：管理后台仪表盘增强（8 指标卡片 + Chart.js 三图表 + 快捷操作 + 系统状态面板）
