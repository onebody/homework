...
## 简介
本仓库中的暑假作业打卡系统在后端采用 SQLAlchemy + Alembic 进行数据库版本管理与增量迁移。系统在容器启动时自动执行“备份 → 迁移 → 种子数据”流程，确保在首次部署、已有数据未追踪版本、以及后续迭代升级等场景下均能安全完成数据库初始化与演进。
...
- 启动编排
  - migrate.py：迁移入口（含备份、create_all 兜底、stamp 初始版本、种子数据）
...
- 应用集成
  ...
  - models.py：完整 ORM 模型集合，作为 Alembic 的元数据来源。
  - config.py：DB_PATH 与 DATABASE_URL 的来源，支持环境变量覆盖。
- 启动编排
  - migrate.py：封装备份、迁移、stamp、create_all 兜底、种子数据逻辑，并在容器 CMD 中自动执行。
...
## 架构总览
下图展示了从容器启动到数据库就绪的整体流程，包括备份、迁移、stamp 与种子数据注入。
...
- migrate.py
  ...
  - stamp 实现：直接操作 sqlite3 写入 alembic_version，避免编码问题。
...
- 建议检查点
  ...
  - 查看容器日志，关注 migrate.py 的输出提示（备份、迁移、stamp、种子数据）。

章节来源
- [migrate.py:72-98](file://summer-homework-checkin/backend/migrate.py#L72-L98)
- [migrate.py:101-125](file://summer-homework-checkin/backend/migrate.py#L101-L125)
- [alembic.ini:1-41](file://summer-homework-checkin/backend/alembic.ini#L1-L41)
- [env.py:1-57](file://summer-homework-checkin/backend/alembic/env.py#L1-L57)

## 结论
该系统的 Alembic 迁移方案兼顾了易用性与健壮性：通过统一的 env.py 与 alembic.ini 管理迁移环境，结合 migrate.py 的“备份 + 智能迁移 + 兜底 + 种子数据”流程，能够在多种部署场景下稳定完成数据库初始化与演进。配合 Docker 卷持久化与 SQLite WAL 模式，整体具备较好的可维护性与并发表现。
