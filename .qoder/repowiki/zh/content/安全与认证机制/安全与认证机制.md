...
- [test_utils.py](file://summer-homework-checkin/tests/test_utils.py)
</cite>

## 更新摘要
**变更内容**   
- **增强速率限制系统**：新增RATE_LIMIT_ENABLED环境变量支持，允许在生产环境和测试环境中灵活控制速率限制功能
- **完善Docker集成**：在docker-compose.yml中添加RATE_LIMIT_ENABLED环境变量配置
- **优化测试环境**：测试套件默认关闭速率限制以提高执行效率
- **提升安全性**：保持原有的防暴力破解能力（登录10次/分钟、注册5次/分钟）
...
图表来源
- [routers/auth.py:1-67](file://summer-homework-checkin/backend/app/routers/auth.py#L1-L67)
...
- [utils/rate_limit.py:1-54](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L1-L54)
...
- [utils/rate_limit.py:9-16](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L9-L16)
- [utils/rate_limit.py:23-28](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L23-L28)
- [utils/rate_limit.py:31-54](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L31-L54)
...
## 架构总览
下图展示从客户端发起登录到受保护资源访问的整体流程，以及增强的可配置速率限制防护机制。
...
Middleware->>Middleware : check_rate_limit(request)
...
- **HMAC令牌签名**
  ...
  - **增强**：签名密钥通过环境变量SUMMER_SECRET配置，支持动态生成和持久化存储
...
图表来源
- [utils/rate_limit.py:9-10](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L9-L10)
- [utils/rate_limit.py:31-54](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L31-L54)
- [main.py:34-42](file://summer-homework-checkin/backend/app/main.py#L34-L42)
- [docker-compose.yml:26-27](file://docker-compose.yml#L26-L27)

章节来源
- [utils/rate_limit.py:1-54](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L1-54)
- [main.py:34-42](file://summer-homework-checkin/backend/app/main.py#L34-L42)
- [docker-compose.yml:26-27](file://docker-compose.yml#L26-L27)
- [test_utils.py:24-26](file://summer-homework-checkin/tests/test_utils.py#L24-L26)
...
## 结论
本系统通过增强的PBKDF2密码哈希、HMAC令牌签名、可配置的速率限制防护、严格的输入校验、ORM 安全查询与四重防代打卡验证，构建了更加完善的安全体系。**最新的安全改进**包括：
...
新引入的可配置速率限制特性显著提升了系统的部署灵活性和安全性，特别是在不同环境下的适应性方面提供了强有力的保障。建议在后续迭代中持续优化人脸模型的可用性与准确率，并考虑引入更细粒度的权限控制和审计日志功能。
