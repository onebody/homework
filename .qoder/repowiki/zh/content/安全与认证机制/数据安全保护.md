...
Svc->>DB : "写入 face_embedding(JSON), face_id_path"
...
- 人脸特征数据
  - 以 JSON 文本存储 512 维向量，存在明文风险。建议对 face_embedding 字段进行加密存储（如 AES-GCM），并在内存中解密处理。
  - 参考路径：[face_embedding 字段:29-29](file://summer-homework-checkin/backend/app/models.py#L29-L29)、[enroll/verify 写入与读取:71-125](file://summer-homework-checkin/backend/app/services/face_service.py#L71-L125)
- 地理位置信息
  - home_lat/home_lng 与打卡 location_lat/location_lng 为明文浮点数。建议对静态常用位置进行加密存储，传输过程强制 HTTPS。
  - 参考路径：[home_lat/home_lng:23-24](file://summer-homework-checkin/backend/app/models.py#L23-L24)、[location_lat/location_lng:79-80](file://summer-homework-checkin/backend/app/models.py#L79-L80)
- 家长手机号
...
- 恶意文件检测
  - 当前未集成病毒扫描或沙箱执行。建议引入外部扫描器（ClamAV）或云函数进行二次检测。
- 存储路径隔离
...
SizeCheck --> |是| Parse["解析 JPEG/PNG 尺寸"]
...
- 策略控制
  ...
  - soft 模式：仅标记高风险但仍记录。
  - 参考路径：[FACE_MODE_ON_ENROLLED:46-49](file://summer-homework-checkin/backend/app/config.py#L46-L49)、[create_checkin 策略分支:116-122](file://summer-homework-checkin/backend/app/services/checkin_service.py#L116-L122)
...
## 结论
当前系统在认证、文件校验、存储隔离与人脸识别方面具备基础安全能力，但在敏感数据加密、上传公开访问控制、日志审计与备份恢复等方面仍有提升空间。建议优先实施敏感字段加密与脱敏、启用 HTTPS、引入上传扫描与鉴权访问、完善审计日志与备份策略，以提升整体数据安全水位。
