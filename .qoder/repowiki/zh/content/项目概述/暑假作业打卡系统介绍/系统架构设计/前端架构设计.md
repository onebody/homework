...
## 更新摘要
**变更内容**
- 更新了Vue.js CDN源从unpkg.com迁移到bootcdn.net，提升国内访问性能
- 优化了静态资源路径解析，使用相对路径支持子路径部署
- 增强了部署灵活性和网络访问稳定性
...
**最新更新**：已优化CDN源和静态资源路径，提升国内访问性能和部署灵活性。
...
subgraph "CDN服务"
BOOTCDN["BootCDN<br/>vue.global.prod.js"]
end
subgraph "后端服务"
...
**图表来源**
- [summer-homework-checkin/frontend/student/index.html:7-8](file://summer-homework-checkin/frontend/student/index.html#L7-L8)
- [summer-homework-checkin/frontend/admin/index.html:7-8](file://summer-homework-checkin/frontend/admin/index.html#L7-L8)
...
**更新**：CDN资源已从unpkg.com迁移至bootcdn.net，显著提升国内访问速度。

章节来源
- [summer-homework-checkin/frontend/student/index.html:1-391](file://summer-homework-checkin/frontend/student/index.html#L1-L391)
- [summer-homework-checkin/frontend/student/app.js:1-403](file://summer-homework-checkin/frontend/student/app.js#L1-L403)
...
- [summer-homework-checkin/frontend/admin/index.html:1-678](file://summer-homework-checkin/frontend/admin/index.html#L1-L678)
- [summer-homework-checkin/frontend/admin/app.js:1-641](file://summer-homework-checkin/frontend/admin/app.js#L1-L641)
- [summer-homework-checkin/frontend/admin/admin.css:1-389](file://summer-homework-checkin/frontend/admin/admin.css#L1-L389)

## 架构总览
整体为前后端分离的 SPA 架构，浏览器直接访问 HTML，通过 BootCDN 引入 Vue 3 运行时，所有业务逻辑在 app.js 中实现。学生端与管理后台各自独立运行，共享同一套后端 API。

```mermaid
sequenceDiagram
participant U as "用户浏览器"
participant V as "Vue 应用(app.js)"
participant C as "BootCDN"
participant F as "fetch 请求"
participant B as "后端 FastAPI"
U->>V : 打开 index.html
V->>C : 加载 Vue 3 运行时
C-->>V : 返回 vue.global.prod.js
...
**图表来源**
- [summer-homework-checkin/frontend/student/index.html:8](file://summer-homework-checkin/frontend/student/index.html#L8)
- [summer-homework-checkin/frontend/admin/index.html:8](file://summer-homework-checkin/frontend/admin/index.html#L8)
...
## 依赖关系分析
- 运行时依赖
  - Vue 3 通过 BootCDN 引入，无需构建工具，版本 3.4.27
- 内部依赖
  - 学生端：index.html 引入 student.css 与 app.js（使用相对路径 ./）
  - 管理后台：index.html 引入 admin.css 与 app.js（使用相对路径 ./）
- 外部依赖
  - 后端 API：/api/* 系列接口，包括认证、打卡、人脸、抽奖、兑换、报表、管理后台等
  - 浏览器 API：FileReader、Geolocation、localStorage

**更新**：静态资源路径已优化为相对路径，支持子路径部署；CDN源已迁移至bootcdn.net。

```mermaid
graph LR
SHTML["student/index.html"] --> SCSS["student/student.css"]
SHTML --> SJS["student/app.js"]
SHTML --> BOOTCDN["BootCDN Vue 3"]
...
- [summer-homework-checkin/frontend/student/index.html:7-8](file://summer-homework-checkin/frontend/student/index.html#L7-L8)
- [summer-homework-checkin/frontend/admin/index.html:7-8](file://summer-homework-checkin/frontend/admin/index.html#L7-L8)
- [summer-homework-checkin/frontend/student/index.html:388](file://summer-homework-checkin/frontend/student/index.html#L388)
- [summer-homework-checkin/frontend/admin/index.html:675](file://summer-homework-checkin/frontend/admin/index.html#L675)
...
- 网络与缓存
  ...
  - **已优化**：Vue.js 运行时已通过 BootCDN 缓存，提升首次加载速度
- 渲染与内存
...
- **新增**：CDN加载问题
  - 现象：Vue.js 运行时加载失败
  - 排查：检查网络连接，确认 BootCDN 可达，或考虑添加备用CDN源
  - 参考路径
    - [summer-homework-checkin/frontend/student/index.html:8](file://summer-homework-checkin/frontend/student/index.html#L8)
    - [summer-homework-checkin/frontend/admin/index.html:8](file://summer-homework-checkin/frontend/admin/index.html#L8)
- **新增**：静态资源路径问题
  - 现象：CSS或JS文件404错误
  - 排查：确认相对路径引用正确，检查部署目录结构
  - 参考路径
    - [summer-homework-checkin/frontend/student/index.html:7](file://summer-homework-checkin/frontend/student/index.html#L7)
    - [summer-homework-checkin/frontend/admin/index.html:7](file://summer-homework-checkin/frontend/admin/index.html#L7)
...
## 结论
该前端架构以"免构建 + CDN"的方式快速落地，学生端与管理后台双端分离，职责清晰、耦合度低。通过统一的 API 集成层与轻量状态管理，实现了稳定的用户交互与审核工作流。**最新优化**包括CDN源迁移至BootCDN提升国内访问性能，以及静态资源路径优化支持子路径部署。后续可在缓存、虚拟化、错误处理与监控方面进一步演进，以提升性能与可维护性。
