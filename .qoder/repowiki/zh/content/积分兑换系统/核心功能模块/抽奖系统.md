...
## 更新摘要
**变更内容**
- 积分系统实现了全新的转盘轮盘机制，替代原有的简单随机抽奖
- 后端新增 `_build_wheel_sectors()` 和 `_pick_from_sectors()` 函数，使用 Fisher-Yates 洗牌算法
- 前端 app.js 进行了重大重构，支持动态转盘渲染和动画效果
- API 响应结构扩展，新增 sectors 数组和 winning_index 字段
- Canvas 尺寸从 320x320 增加到 360x360 像素，动画时长调整为 5-6 秒
...
## 简介
本仓库包含两套"抽奖"相关实现：
- **积分体系下的抽奖子系统（points-system）**：经过重大重构，现已实现全新的转盘轮盘机制，提供视觉化的抽奖体验。
- **暑期打卡系统中的抽奖模块（summer-homework-checkin）**：基于用户"抽奖资格"字段进行抽奖，支持按概率权重抽取奖品并生成中奖通知与兑换记录。

本文围绕以下目标展开：
- 深入解析加权随机算法的实现原理（概率计算、权重分配、随机数生成）。
- 详细说明转盘轮盘机制的实现，包括扇区生成、Fisher-Yates 洗牌算法和动画效果。
- 说明抽奖券发放规则、使用条件与库存管理逻辑。
- 设计并文档化抽奖结果数据模型与历史记录追踪机制。
- 解释抽奖资格验证流程（含角色限制、次数限制等业务规则）。
- 提供 API 接口文档、错误处理与性能优化建议。
...
- [points-system/frontend/styles.css:1-212](file://points-system/frontend/styles.css#L1-212)
...
## 核心组件
- **路由层**
  - 积分体系：提供奖池查询、转盘抽奖、抽奖历史查询。
  - 打卡系统：提供抽奖资格与记录查询、抽奖入口。
- **服务层**
  - 积分体系：负责积分兑换抽奖券、转盘抽奖核心逻辑（含转盘扇区生成、Fisher-Yates 洗牌、加权随机选择、并发锁、事务一致性、库存扣减、流水落库）。
  - 打卡系统：负责消耗抽奖资格、按概率权重随机选奖、创建中奖兑换记录与通知。
- **数据模型**
  - 积分体系：用户、积分账户、积分流水、抽奖券流水、抽奖奖池、抽奖记录。
  - 打卡系统：用户（含抽奖资格字段）、奖品（含概率与库存）、抽奖记录、兑换记录。
- **模式定义**
  - 统一了请求与响应的数据结构，新增转盘扇区相关的数据结构。
- **前端交互**
  - 积分体系：Canvas 转盘渲染、动态扇区绘制、旋转动画、结果弹窗展示。
...
- [points-system/backend/app/schemas.py:1-157](file://points-system/backend/app/schemas.py#L1-157)
- [summer-homework-checkin/backend/app/schemas.py:1-322](file://summer-homework-checkin/backend/app/schemas.py#L1-L322)

## 架构总览
整体分层清晰：前端通过 HTTP 调用路由，路由校验参数与权限后委托服务层执行业务逻辑，服务层操作数据库模型并返回结构化响应。**积分体系现已升级为转盘机制，提供完整的转盘配置数据和动画控制信息。**
...
### 转盘轮盘机制实现原理
**更新** 积分体系现已实现完整的转盘轮盘机制，替代原有的简单随机抽奖。

#### 后端转盘扇区生成
- **扇区构建**：`_build_wheel_sectors()` 函数生成包含 15 个真实奖品 + 3-5 个"谢谢参与"的动态转盘
- **Fisher-Yates 洗牌**：确保扇区顺序的随机性，避免固定模式
- **权重分配**：真实奖品使用原 weight 字段，"谢谢参与"固定权重为 2

#### 前端转盘渲染与动画
- **Canvas 绘制**：支持 18-20 个扇区的动态渲染，Canvas 尺寸 360x360 像素
- **旋转动画**：CSS transform 实现平滑旋转，时长 5-6 秒，缓动函数 cubic-bezier(0.17, 0.67, 0.12, 0.99)
- **指针定位**：顶部指针（12点方向），精确计算目标扇区中心角度
- **结果展示**：动画结束后弹出中奖结果弹窗

```mermaid
flowchart TD
Start(["开始转盘抽奖"]) --> BuildSectors["生成转盘扇区<br/>15个真实奖品 + 3-5个谢谢参与"]
BuildSectors --> Shuffle["Fisher-Yates 洗牌算法<br/>确保随机性"]
Shuffle --> PickWinner["加权随机选择<br/>确定中奖扇区索引"]
PickWinner --> DrawWheel["前端绘制转盘<br/>18-20个扇区"]
DrawWheel --> Spin["播放旋转动画<br/>5-6秒，多圈旋转"]
Spin --> ShowResult["显示中奖结果<br/>弹窗提示"]
ShowResult --> End(["结束"])
```
...
- **积分体系前端依赖**：app.js 依赖后端 API 返回的转盘数据，styles.css 提供转盘样式和动画效果。
...
- **前端性能优化**
  - Canvas 渲染优化：使用 requestAnimationFrame 和 CSS transform 提升动画性能。
...
## 结论
两套抽奖实现均以"候选集过滤 + 累计权重区间选择"的加权随机为核心，结合库存管理与事务一致性保障，提供了稳定可靠的抽奖体验。**积分体系现已升级为转盘轮盘机制，通过 Fisher-Yates 洗牌算法确保随机性，提供视觉化的抽奖体验，而打卡系统则以"用户资格字段 + 角色限制"为主。** 

建议在大规模部署时引入数据库行级锁与外部缓存，并对随机算法进行进一步优化以提升可扩展性与性能。对于转盘机制，可以考虑使用 WebGL 渲染优化大量扇区时的性能表现。