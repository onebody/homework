...
## 更新摘要
**所做更改**   
- 更新了积分计算系统，现在基于站点配置进行动态积分值计算
- 服务层新增对站点配置参数的读取和应用能力
- 完善了积分发放机制的配置化支持
- 增强了打卡服务的灵活性和可配置性
...
## 核心组件
- 打卡服务（checkin_service）：封装正常打卡、补卡、审核通过/拒绝、连续天数重算、**基于配置的积分发放**、抽奖资格解锁与通知。
...
**更新** 打卡服务现已集成新的积分计算系统，支持从站点配置中读取和应用的动态积分值。
...
- 管理端审核：路由获取待审记录 → 调用打卡服务批准/拒绝 → **基于站点配置发放积分** → 重算连续天数与抽奖资格 → 发送通知。
...
participant C as "站点配置<br/>site_config"
...
S->>C : 读取站点积分配置
C-->>S : {checkin_points, makeup_points}
S->>DB : 持久化打卡记录(待审核)
S->>N : 通知学生与家长
S-->>R : 返回打卡记录
R-->>U : 200 OK
...
- 审核通过：标记有效、**基于站点配置发放积分**、重算连续天数与抽奖资格、发送通知。
- 审核拒绝：标记无效、发送通知。
- 连续天数与里程碑：按有效打卡日期排序计算当前连续与历史最长；每达到 7 天里程碑自动发放抽奖券并记录通知。

**更新** 积分发放机制现已完全基于站点配置进行动态计算，支持不同站点使用不同的积分策略。
...
Persist --> LoadConfig["加载站点积分配置"]
LoadConfig --> CalculatePoints["计算应得积分"]
CalculatePoints --> Notify["通知学生与家长"]
...
- **更新的积分发放**：正常打卡与补卡分别对应不同分值，现从站点配置中动态读取 CHECKIN_POINTS 和 MAKEUP_POINTS 参数。
...
- 管理端审核：PUT /api/admin/checkins/{id}/review，支持 approved/rejected，批准后**基于站点配置自动发放积分并重算连续天数**。
...
- 地理位置风险：
  ...
  - 参考：[geo.py:6-24](file://summer-homework-checkin/backend/app/utils/geo.py#L6-L24)、[config.py:27-28](file://summer-homework-checkin/backend/app/config.py#L27-28)
- 补卡失败：
...
- 审核未生效：
  ...
  - **新增**：检查站点配置中的积分参数是否正确设置。
  - 参考：[routers/admin.py:84-103](file://summer-homework-checkin/backend/app/routers/admin.py#L84-L103)、[checkin_service.py:166-191](file://summer-homework-checkin/backend/app/services/checkin_service.py#L166-191)
- **新增**：积分计算异常：
  - 现象：积分发放不正确或为空。
  - 排查：检查站点配置中的 CHECKIN_POINTS 和 MAKEUP_POINTS 参数；确认配置加载是否正常。
  - 参考：[checkin_service.py:166-209](file://summer-homework-checkin/backend/app/services/checkin_service.py#L166-209)
...
- [image.py:51-61](file://summer-homework-checkin/backend/app/utils/image.py#L51-L61)
- [config.py:27-33](file://summer-homework-checkin/backend/app/config.py#L27-33)
- [geo.py:6-24](file://summer-homework-checkin/backend/app/utils/geo.py#L6-L24)
- [checkin_service.py:72-103](file://summer-homework-checkin/backend/app/services/checkin_service.py#L72-L103)
- [routers/admin.py:84-103](file://summer-homework-checkin/backend/app/routers/admin.py#L84-L103)
- [checkin_service.py:166-191](file://summer-homework-checkin/backend/app/services/checkin_service.py#L166-191)
...
- 完善的通知体系，及时同步学生与家长。
- **新增**：基于站点配置的动态积分计算系统，提升了系统的灵活性和可配置性。

在生产环境中，建议引入异步通知、缓存与更强的数据库支撑，进一步提升稳定性与扩展性。新的积分计算系统为未来可能的多站点部署和差异化运营策略提供了良好的技术基础。
...
- **更新的积分规则**：CHECKIN_POINTS、MAKEUP_POINTS（现从站点配置动态读取）
