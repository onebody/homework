...
## 更新摘要
**变更内容**   
- 新增密码修改API接口 /api/auth/password，支持所有已登录用户角色进行密码修改
- 实现了完整的密码验证机制，包括原密码验证和新密码长度校验
- 采用与注册相同的PBKDF2-SHA256密码哈希策略确保安全性
- 返回友好的成功消息提示用户下次登录使用新密码
...
## 简介
本文件为用户认证相关路由的技术文档，覆盖注册、登录、获取当前用户和密码修改等核心认证流程；阐述令牌生成、校验机制（基于 HMAC 的无状态 token）；说明密码哈希策略与安全防护要点；提供多角色访问控制的路由装饰器用法（学生/家长/管理员）；并给出请求参数验证规则、错误响应格式规范以及调用示例路径。**现已集成速率限制中间件，有效防护暴力破解攻击，保障认证接口的安全性，并新增了安全的密码修改功能。**
...
## 架构总览
认证流程涉及"速率限制 → 路由 → 安全 → 依赖注入 → 数据库"的协作。下图展示注册、登录、密码修改的关键时序，包括新的速率限制保护机制。
...
## 结论
本认证子系统采用轻量无状态令牌与 PBKDF2 密码哈希，结合 FastAPI 依赖注入实现了注册、登录、当前用户读取和密码修改的核心能力。**新增的密码修改接口提供了安全的密码更新机制，支持所有已登录用户角色，包含完整的原密码验证和新密码长度校验。** **新增的速率限制中间件有效防护了暴力破解攻击，通过内存级频率控制和差异化限制策略，显著提升了系统的安全性。** 通过 require_role 装饰器可扩展多角色访问控制。当前未实现登出与刷新令牌，建议后续补充刷新令牌机制与更严格的密码策略（最小长度、复杂度校验等）。
...
- **密码修改（新增）**
  - 参考实现位置：[summer-homework-checkin/backend/app/routers/auth.py:56-67](file://summer-homework-checkin/backend/app/routers/auth.py#L56-L67)
  - 入参模型：[summer-homework-checkin/backend/app/schemas.py:40-43](file://summer-homework-checkin/backend/app/schemas.py#L40-L43)
  - 安全验证：[summer-homework-checkin/backend/app/security.py:22-24](file://summer-homework-checkin/backend/app/security.py#L22-L24)
