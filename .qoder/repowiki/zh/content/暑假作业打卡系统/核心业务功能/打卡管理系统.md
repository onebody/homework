...
## 更新摘要
**变更内容**   
- 新增速率限制中间件，为敏感接口提供防暴力破解保护
- 扩展速率限制覆盖范围，包括密码修改、人脸数据采集和打卡接口等关键端点
- 实现基于IP和用户ID的细粒度限流策略
- 添加请求频率监控和异常访问检测机制
- 优化安全架构，防止恶意攻击和滥用行为
...
## 简介
本技术文档面向"暑假作业打卡系统"的开发者与维护者，围绕正常打卡与补卡两种模式，系统性阐述四重防作弊机制（照片合规校验、地理位置验证、人脸识别比对、场景风险判定）、连续打卡统计算法、审核流程状态转换、积分发放与抽奖资格解锁规则，以及补卡业务规则的配置化管理。同时新增挑战赛功能，支持学生参与各种学习挑战活动，通过统一的前端界面实现打卡与挑战的双模式切换。**最新更新**：系统已全面增强安全防护能力，新增速率限制机制以防止暴力破解攻击，覆盖密码修改、人脸数据采集和打卡接口等敏感端点。

## 项目结构
后端采用 FastAPI + SQLAlchemy + SQLite 的轻量架构，路由层按功能划分，服务层封装核心业务，工具层提供图像解析、地理距离计算、文件存储与**速率限制**能力。前端静态资源通过挂载方式提供服务，新增的挑战赛功能通过内部标签切换机制集成到统一界面。
...
C --> D["工具层<br/>image.py / geo.py / storage.py / rate_limit.py"]
...
- [backend/app/utils/rate_limit.py:1-100](file://summer-homework-checkin/backend/app/utils/rate_limit.py#L1-100)
...
- 工具层
  ...
  - **新增** 速率限制工具：基于IP和用户ID的请求频率控制，防止暴力破解攻击。
- 配置与数据
...
## 架构总览
系统以 FastAPI 为入口，路由分发到服务层执行核心逻辑；服务层调用工具层完成图像解析、地理计算、文件存储与**速率限制检查**，并通过通知服务推送消息；所有持久化操作通过 SQLAlchemy 会话访问 SQLite 数据库。新增的挑战赛功能通过独立的路由和服务层实现，与打卡功能共享基础设施。**新增的速率限制中间件**在请求进入路由前进行频率检查，有效防止暴力破解攻击。
...
Note over Client,DB : 带速率限制的打卡流程
...
## 结论
本系统通过照片合规、地理位置、人脸识别与场景风险判定的四重机制，结合严格的补卡规则与审核流程，构建了较为完善的防作弊体系。**最新更新**：系统已全面增强安全防护能力，新增的速率限制机制有效防止了暴力破解攻击，覆盖了密码修改、人脸数据采集和打卡接口等敏感端点。新增的挑战赛功能为学生提供了更多元化的学习激励方式，通过统一的前端界面实现了打卡与挑战的双模式无缝切换。连续打卡统计算法清晰可靠，积分与抽奖资格发放遵循明确的里程碑策略。整体架构简洁、模块化良好，便于扩展与维护，**新增的安全防护机制进一步提升了系统的稳定性和安全性**。
