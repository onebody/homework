...
## 更新摘要
**变更内容**   
- 新增AES-CTR加密模块用于人脸特征数据静态保护
- 集成cryptography==43.0.1依赖实现生物识别数据安全存储
- 更新数据存储流程以支持加密的人脸嵌入向量
- 增强安全配置管理，包含密钥管理和加密参数配置
...
- **新增** 安全加密层：负责人脸特征的AES-CTR加密和解密操作
...
- 人脸服务（face_service.py）
  ...
  - **新增** 加密存储：使用AES-CTR算法对人脸特征进行加密后存储
...
## 架构总览
下图展示从客户端到后端的人脸识别关键路径：采集、比对、打卡集成与健康检查，以及新增的加密保护流程。
...
Service->>Crypto : encrypt(embedding)
Crypto-->>Service : encrypted_embedding
Service->>Storage : save_upload(bytes, user.id, "face")
Service->>Service : store_encrypted_embedding(encrypted_embedding)
...
Service->>Service : _embed(bytes) -> 检测+特征
Service->>Crypto : decrypt(stored_embedding)
...
- **新增** 加密存储（_encrypt_embedding）
  - 使用AES-CTR算法对512维人脸特征向量进行加密
  - 生成随机IV（初始化向量）确保相同明文产生不同密文
  - 将加密后的数据和IV一起存储到数据库
- 注册（enroll）
  - 要求检测到且仅检测到一张人脸
  - 保存原始照片与加密后的embedding到数据库
- 1:1 比对（verify）
  - 读取用户已存加密embedding并解密
...
ExtractEmb --> Encrypt["AES-CTR 加密 embedding"]
...
- **新增** 加密配置：FACE_ENCRYPTION_KEY、FACE_ENCRYPTION_SALT、FACE_ENCRYPTION_MODE
...
### AES-CTR加密实现
系统集成了cryptography==43.0.1库来实现AES-CTR对称加密算法，确保人脸特征数据在静态存储时的安全性。

**加密流程**
1. **密钥生成**：使用安全的随机数生成器创建AES密钥
2. **IV生成**：为每次加密生成唯一的初始化向量（IV）
3. **数据加密**：对512维人脸特征向量进行AES-CTR加密
4. **数据存储**：将加密数据和IV一起存储到数据库

**解密流程**
1. **数据检索**：从数据库获取加密数据和对应的IV
2. **密钥获取**：从安全配置中获取AES密钥
3. **数据解密**：使用相同的密钥和IV进行解密
4. **特征比对**：使用解密后的原始特征进行相似度计算

```mermaid
flowchart LR
A["原始人脸特征<br/>512维向量"] --> B["AES-CTR加密器"]
B --> C["加密数据<br/>密文"]
B --> D["初始化向量IV"]
C --> E["数据库存储"]
D --> E
F["数据库检索"] --> G["解密器"]
G --> H["原始人脸特征<br/>512维向量"]
H --> I["相似度计算"]
```
...
- **FACE_ENCRYPTION_KEY**：AES加密主密钥，应存储在安全的位置
- **FACE_ENCRYPTION_SALT**：用于密钥派生的盐值
- **FACE_ENCRYPTION_MODE**：加密模式（默认使用CTR模式）
- **FACE_ENCRYPTION_IV_LENGTH**：初始化向量长度（通常16字节）

### 密钥管理策略
- **密钥存储**：使用环境变量或安全配置管理服务存储密钥
- **密钥轮换**：支持定期更换加密密钥
- **向后兼容**：支持旧版本加密数据的迁移和解密
...
- 运行时依赖
  ...
  - **新增** cryptography==43.0.1：提供AES-CTR加密功能
- 模块耦合
  - 路由层依赖服务层
  - 服务层依赖工具层、配置层和加密层
  - 加密层依赖cryptography库
...
C --> E["安全随机数生成"]
C --> F["密钥管理"]
G["face_service.py"] --> H["config.py"]
G --> I["image.py"]
G --> J["storage.py"]
G --> K["cryptography"]
L["verification_service.py"] --> G
M["checkin_service.py"] --> L
N["face.py"] --> G
O["checkin.py"] --> M
```
...
- 向量存储
  ...
  - **新增** 加密数据压缩：对加密后的数据进行压缩存储
- 图像预处理
  - 轻量解析减少第三方库依赖，降低内存占用
- 降级策略
  - 模型不可用时返回明确状态，结合 enforce/soft 模式保障业务连续性
  - **新增** 加密失败降级：加密服务不可用时记录日志并回退到明文存储（需配置开关）
...
## 结论
本系统通过 InsightFace 预训练模型实现了稳定的 1:1 人脸比对能力，并与打卡流程深度集成。**新增的AES-CTR加密机制**确保了人脸特征数据在静态存储时的安全性，符合生物识别数据的隐私保护要求。通过严格的图像质量校验、多维度风险判定、明确的降级策略和强大的加密保护，系统在 AI 服务不可用时仍能保持可控与可审计。未来可扩展至 1:N 检索，只需扩展比对逻辑而不改动主流程。
