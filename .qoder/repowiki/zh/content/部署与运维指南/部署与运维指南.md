...
- [homework.conf](file://nginx/sites/homework.conf)
- [points.conf](file://nginx/sites/points.conf)
</cite>

## 更新摘要
**所做变更**
- **部署脚本关键修复**：改进了systemd管理的容器环境支持，包括systemd单元检测、fail-fast行为增强
- **CORS白名单配置优化**：添加了默认值支持和更灵活的环境变量处理
- **WAL一致性备份增强**：提升了数据库备份的可靠性和数据一致性保证
- **系统服务集成改进**：增强了与systemd服务的集成和状态监控能力
...
- **重大更新** 环境变量配置管理：严格的安全配置要求，强制环境变量注入，密钥强度验证
- 日志收集、性能监控、错误追踪的配置方法
- 备份恢复策略、扩容升级方案
- PWA 应用配置、静态资源优化和CDN部署的最佳实践
- **增强** 代码仓库卫生管理与版本控制最佳实践，包括敏感文件保护和IDE配置排除
- **新增** 安全的测试工作流和环境变量管理
- **最新** systemd管理的容器环境支持改进，提升生产环境稳定性
...
O["Nginx 配置<br/>default.conf / sites/*"]
P["Systemd集成<br/>服务管理"]
end
A --> B
B --> C
...
- [default.conf:1-50](file://nginx/default.conf#L1-L50)
- [homework.conf:1-30](file://nginx/sites/homework.conf#L1-L30)
- [points.conf:1-30](file://nginx/sites/points.conf#L1-L30)
...
- **重大更新** 安全配置验证：严格的环境变量注入要求，密钥强度验证，防止不安全部署
...
- **新增** Nginx模块化配置：支持多站点部署和灵活的代理配置
- **最新** systemd服务集成：增强的容器环境管理和故障恢复机制
...
## 架构总览
系统由 FastAPI 作为 API 与服务端渲染入口，前端静态页面通过 StaticFiles 直接托管；数据库默认 SQLite，支持热切换至关系型数据库；**新增** 自动化迁移系统确保数据库版本一致性；**新增** 子路径部署支持，前端自动检测基础路径并调整所有资源URL；**重大更新** 严格的安全配置验证机制，确保所有敏感信息通过环境变量注入；人脸识别使用 insightface 本地推理，具备自动降级能力；**新增** Nginx模块化配置架构，支持多站点部署；**最新** systemd服务集成提升生产环境稳定性。

```mermaid
graph TB
Client["浏览器/移动端"] --> Nginx["Nginx 反向代理<br/>模块化配置"]
...
subgraph "安全配置验证"
EnvCheck["环境变量验证"] --> KeyValidation["密钥强度检查"]
KeyValidation --> ConfigLoad["配置加载"]
ConfigLoad --> App
end
subgraph "Nginx模块化配置"
DefaultConf["default.conf"] --> SitesConf["sites/*.conf"]
SitesConf --> Proxy["反向代理配置"]
Proxy --> FastAPI
end
subgraph "Systemd服务管理"
Systemd["Systemd 服务"] --> Container["容器管理"]
Container --> HealthCheck["健康检查"]
HealthCheck --> AutoRestart["自动重启"]
end
Deploy --> Client
```
...
为了提高开发工作流效率和安全性，项目采用了严格的版本控制策略和安全配置要求：
- 根目录 `.gitignore` 配置排除了 `snake-game/` 目录和所有敏感文件
- **安全增强**：明确排除 `.secret_key`、`.env`、数据库文件等敏感信息
- **严格的环境变量要求**：所有敏感配置必须通过环境变量注入，不再支持硬编码默认值
- 各子项目独立的 `.gitignore` 配置确保运行时文件和敏感信息不被提交
- **版本控制优化**：更新的 .gitignore 配置现在包含对 IDE 设置和临时验证工件的排除，提升团队开发环境一致性
...
- **日志输出**：详细的彩色日志输出和错误提示
- **安全验证**：部署前检查环境变量配置完整性
- **Systemd集成**：增强的systemd服务检测和故障恢复机制
...
participant Systemd as "Systemd 服务"
participant Docker as "Docker 容器"
participant DB as "数据库"
participant Health as "健康检查"
Dev->>Script : 执行 deploy.sh
Script->>Script : 环境变量验证
Script->>Systemd : 检测systemd服务状态
Systemd-->>Script : 返回服务状态
Script->>DB : 备份数据库
...
### Docker 容器化部署
**重大更新** 增强的安全配置和优化的容器启动流程

Docker 配置文件已更新，确保数据库在应用启动前就绪，并移除了所有硬编码的敏感信息：

#### 启动流程
1. **依赖安装**：使用 requirements.docker.txt 精简依赖
2. **源码复制**：复制后端和前端代码
3. **环境变量验证**：检查必需的环境变量是否配置
4. **数据库准备**：执行 migrate.py 进行迁移和种子数据
5. **服务启动**：启动 Uvicorn 服务
...
- **新增** 自动化部署流程
  ...
  - 增量升级，无需停机维护
  - 自动健康检查和回滚机制
  - **重大更新** 环境变量验证，确保所有敏感配置正确注入
  - **最新** systemd服务集成，提升生产环境稳定性
- **新增** 子路径部署配置
  - 支持在 `/homework` 等子路径下部署
  - 前端自动检测基础路径，无需手动配置
  - Nginx反向代理需要正确转发子路径
- **新增** Nginx模块化配置
  - 使用 default.conf 作为主配置文件
  - 通过 sites/*.conf 模块化配置不同站点
  - 支持多应用共存和灵活的路由配置
- 反向代理（Nginx）
...
### 环境变量配置管理
**重大更新** 严格的环境变量注入要求和密钥强度验证

以下关键参数均支持通过环境变量覆盖，便于不同环境差异化配置。**所有敏感配置现在必须通过环境变量注入，不再支持硬编码默认值**：

#### 安全相关配置（必须通过环境变量注入）
- **SUMMER_SECRET**：签名密钥（生产务必通过环境变量注入，禁止使用默认值）
...
系统现在实现了健壮的密钥强度验证机制，防止使用不安全的默认值进行部署：
- **SUMMER_SECRET** 必须满足最小长度要求
- **ADMIN_INIT_PASSWORD** 必须设置为非空值
- 部署时会验证所有必需的环境变量是否已正确配置
- 任何缺失或不安全的配置都会导致部署失败
...
**章节来源**
- [test_review.py:24-51](file://summer-homework-checkin/test_review.py#L24-51)
- [seed.py:62-79](file://summer-homework-checkin/backend/seed.py#L62-79)

### Nginx模块化配置架构
**新增** 模块化的Nginx配置系统

项目现在采用模块化的Nginx配置架构，支持多站点部署和灵活的代理配置：

#### 配置结构
- **default.conf**：主配置文件，包含全局设置和基本配置
- **sites/**：模块化站点配置文件目录
  - **homework.conf**：作业打卡系统的站点配置
  - **points.conf**：积分系统的站点配置

#### 模块化优势
- **配置分离**：不同站点的配置相互独立，便于维护
- **灵活扩展**：可以轻松添加新的站点配置
- **环境隔离**：不同环境的配置可以独立管理
- **团队协作**：多人可以同时编辑不同的配置文件
...
- 模块耦合：
  ...
  - **新增** Nginx模块化配置：default.conf 和 sites/*.conf
  - **最新** systemd服务集成：增强容器环境管理
...
StudentJS --> FixUrl
Nginx["nginx/*"] --> Deploy["deploy.sh"]
Deploy --> Docker["docker-compose.yml"]
Deploy --> Systemd["systemd服务"]
Systemd --> Container["容器管理"]
```
...
- **新增** 部署优化
  ...
  - 增量部署减少停机时间
  - 子路径部署减少配置复杂度
  - **重大更新** 环境变量验证在启动时快速失败，避免不必要的资源消耗
  - **最新** systemd服务集成提升生产环境稳定性和故障恢复能力
...
- 常见问题定位
  ...
  - **新增** 子路径部署问题：检查Nginx模块化配置是否正确加载
  - **新增** API请求失败：确认子路径下API调用是否正确拼接BASE_PATH
  - **新增** 数据库迁移问题：检查 migrate.py 日志和 alembic_version 表
  - **新增** 部署失败：查看部署脚本日志和健康检查结果
  - **重大更新** 环境变量配置问题：检查所有必需的环境变量是否正确设置，特别是 SUMMER_SECRET 和 ADMIN_INIT_PASSWORD
  - **版本控制问题**：检查 .gitignore 配置是否正确排除 IDE 设置和临时文件
  - **最新** systemd服务问题：检查systemd服务状态和容器运行状态
- 日志与监控
  ...
  - **重大更新** 环境变量验证日志：部署时输出详细的环境变量配置状态
  - **新增** Nginx配置调试：检查default.conf和sites/*.conf的语法正确性
  - **最新** systemd日志：检查systemd服务日志和容器状态
- 备份与恢复
...
- **增强** 版本控制问题排查
  ...
  - **版本控制优化**：确认新的 .gitignore 配置正确排除了 IDE 设置和临时验证工件
- **新增** 子路径部署问题排查
  - 检查浏览器控制台的网络请求，确认API路径是否正确
  - 验证Nginx子路径代理配置是否正确转发请求
  - 检查静态资源URL是否包含正确的基础路径前缀
  - 确认fixUrl()方法在不同浏览器中的兼容性
  - **新增** 检查Nginx模块化配置的加载顺序和语法
- **重大更新** 环境变量配置问题排查
  - 检查所有必需的环境变量是否已正确设置
  - 验证密钥强度是否符合安全要求
  - 确认环境变量注入机制正常工作
  - 检查部署脚本的环境变量验证输出
  - **新增** 检查Docker环境变量传递是否正确
- **最新** systemd服务问题排查
  - 检查systemd服务状态：`systemctl status <service-name>`
...
## 结论
本指南提供了从开发到生产的完整部署与运维方案，涵盖环境搭建、**全新的自动化部署流程**、反向代理、多进程、**重大更新的环境变量安全管理**、日志监控、备份恢复以及 PWA 与静态资源优化。**增强的版本控制最佳实践**帮助团队维护代码仓库的整洁性和安全性，**新的安全测试工作流**确保了测试过程的安全性。**新增的自动化迁移系统和部署脚本**大幅简化了生产环境的运维复杂度，提高了部署的可靠性和效率。**新增的子路径部署支持**使得系统能够在各种复杂的部署环境中灵活运行，包括Docker容器、反向代理和云平台路由等场景。**重大更新的安全配置验证机制**确保了所有敏感信息都通过环境变量注入，防止了不安全的默认配置导致的潜在安全风险。**版本控制优化**通过更新的 .gitignore 配置提升了团队开发环境的一致性，确保 IDE 设置和临时文件不会被提交到版本控制。**新增的Nginx模块化配置架构**为多站点部署提供了灵活的解决方案，支持不同应用的独立配置和管理。**最新的systemd服务集成改进**进一步提升了生产环境的稳定性和可靠性，增强了故障恢复和服务管理能力。按照本文实践，可在保证稳定性的同时获得良好的用户体验与可观测性。

## 附录

### 环境变量清单与说明
**重大更新** 所有敏感配置现在必须通过环境变量注入

- **SUMMER_SECRET**：签名密钥（生产必须通过环境变量注入，禁止使用默认值）
...
#### 环境变量配置（必须通过环境变量注入）
- [ ] 设置了强随机 SUMMER_SECRET 环境变量（禁止使用默认值）
...
- [ ] 确认了健康检查端点可用
- [ ] 准备了回滚方案和应急联系人
- [ ] **重大更新** 验证了环境变量注入机制正常工作

#### **新增** Nginx配置检查
- [ ] 验证default.conf语法正确性
- [ ] 检查sites/*.conf配置文件加载顺序
- [ ] 确认模块化配置的正确性
- [ ] 测试Nginx服务的启动和重载

#### **重大更新** 环境变量验证检查
- [ ] 确认所有必需的环境变量都已正确设置
- [ ] 验证 SUMMER_SECRET 满足最小长度要求
- [ ] 检查 ADMIN_INIT_PASSWORD 是否为非空值
- [ ] 测试环境变量验证在部署时的行为
- [ ] 确认没有使用任何硬编码的默认密钥
...
# 2. 设置必需的环境变量
export SUMMER_SECRET=$(openssl rand -hex 32)
export ADMIN_INIT_PASSWORD="your_secure_password"
...
# 2. 设置生产环境变量
export SUMMER_SECRET=$(openssl rand -hex 32)
export ADMIN_INIT_PASSWORD="production_secure_password"
...
# 2. 验证密钥强度
if [ ${#SUMMER_SECRET} -lt 32 ]; then
    echo "警告：SUMMER_SECRET 长度不足32位"
fi

# 3. 检查环境变量注入
docker inspect summer-homework | grep -A 10 "Environment"

# 4. 验证部署脚本的环境变量验证
./scripts/deploy.sh local 2>&1 | grep -i "environment\|secret\|password"
```
