...
## 更新摘要
**所做变更**
- **重大改进**：新增自动化数据库迁移系统，支持增量升级和数据保护
- **新增** 自动化部署脚本，支持本地和生产环境的无缝部署
- **增强** Docker 启动流程，确保数据库在应用启动前就绪
- **完善** 生产环境部署策略，包含自动备份和回滚机制
- **优化** 版本控制安全配置，防止敏感文件意外泄露
- **改进** 测试工作流以支持基于环境变量的凭据管理
...
## 项目结构
后端采用 FastAPI + SQLAlchemy + Alembic 迁移 + SQLite（可替换为 PostgreSQL/MySQL），前端为免构建的 Vue3 H5 与管理后台，通过后端静态挂载提供。
...
subgraph "部署工具"
I["部署脚本<br/>scripts/deploy.sh"]
J["Docker 配置<br/>Dockerfile / docker-compose.yml"]
K["Docker 依赖<br/>requirements.docker.txt"]
end
subgraph "前端"
L["学生端 H5<br/>frontend/student/index.html"]
M["管理后台<br/>frontend/admin/index.html"]
end
A --> B
B --> C
C --> D
...
## 架构总览
系统由 FastAPI 作为 API 与服务端渲染入口，前端静态页面通过 StaticFiles 直接托管；数据库默认 SQLite，支持热切换至关系型数据库；**新增** 自动化迁移系统确保数据库版本一致性；人脸识别使用 insightface 本地推理，具备自动降级能力。
...
- [env.py:1-57](file://summer-homework-checkin/backend/alembic/env.py#L1-57)
- [001_initial.py:1-183](file://summer-homework-checkin/backend/alembic/versions/001_initial.py#L1-183)
...
**章节来源**
- [deploy.sh:1-163](file://scripts/deploy.sh#L1-163)
...
**章节来源**
- [Dockerfile:1-22](file://summer-homework-checkin/Dockerfile#L1-22)
- [docker-compose.yml:1-59](file://docker-compose.yml#L1-59)
- [requirements.docker.txt:1-15](file://summer-homework-checkin/backend/requirements.docker.txt#L1-15)
...
## 结论
本指南提供了从开发到生产的完整部署与运维方案，涵盖环境搭建、**全新的自动化部署流程**、反向代理、多进程、环境变量调优、日志监控、备份恢复以及 PWA 与静态资源优化。**增强的版本控制最佳实践**帮助团队维护代码仓库的整洁性和安全性，**新的安全测试工作流**确保了测试过程的安全性。**新增的自动化迁移系统和部署脚本**大幅简化了生产环境的运维复杂度，提高了部署的可靠性和效率。按照本文实践，可在保证稳定性的同时获得良好的用户体验与可观测性。
