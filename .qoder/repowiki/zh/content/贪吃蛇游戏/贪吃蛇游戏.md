...
## 简介
本项目是一个基于原生 JavaScript 与 HTML5 Canvas 的完整贪吃蛇游戏。它采用模块化组织，围绕一个主引擎 GameEngine 驱动游戏循环、状态机、输入处理、碰撞检测、渲染与持久化等核心能力；同时提供多模式（经典、限时、障碍）、难度调节、成就解锁、音效管理、UI 菜单与 HUD 等高级特性。文档将系统解析引擎设计模式、蛇移动算法、食物随机生成策略、Canvas 渲染优化、事件响应机制，并给出可扩展性建议与最佳实践。
